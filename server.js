// ============================================================
//  server.js  —  SaarthiX Groww Live Data Proxy Backend
// ============================================================
...
// Swapping in Groww's automatic refresh flow will just modify this function.
...
function fetchFromGroww(url) {
  return new Promise((resolve, reject) => {
    ...
    if (!token) {
      return reject({ status: 401, message: 'GROWW_ACCESS_TOKEN is missing in server .env configuration.' });
    }
...
// Input: query parameter `symbols` (e.g. RELIANCE,TCS,INFY)
// Output: mapped { prices: { RELIANCE: { price, change, changePct }, ... }, timestamp }
app.get('/api/ltp', async (req, res) => {
  try {
    ...
    if (data && Array.isArray(data)) {
      data.forEach(item => {
        // Strip both "NSE_" and "BSE_" prefixes
...
// Input: query parameter `symbol` (e.g. RELIANCE), optional `exchange` (default NSE)
app.get('/api/quote', async (req, res) => {
  try {
    ...
    // Groww quote payload mapping: payload.last_price, payload.last_trade_time, payload.day_change, payload.day_change_perc
    if (data && data.payload) {
      const p = data.payload;
      
      // Convert epoch ms to readable IST string
      let formattedTime = '—';
...
// Fallback to serve index/login page
app.get('*', (req, res) => {
